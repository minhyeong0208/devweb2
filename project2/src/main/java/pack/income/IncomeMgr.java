...
public class IncomeMgr {
    ...
    private PreparedStatement pstmt; // SQL 쿼리 실행을 위한 PreparedStatement 객체
    private ResultSet rs; // 쿼리 결과를 저장하는 ResultSet 객체
    private DataSource ds; // 데이터베이스 연결 풀을 관리하는 DataSource 객체

	public IncomeMgr() {
		try {
			Context context = new InitialContext(); // JNDI 초기 컨텍스트 생성
			ds = (DataSource) context.lookup("java:comp/env/jdbc_maria"); // 데이터 소스 탐색 및 설정
		} catch (Exception e) {
			System.out.println("DB 연결 실패 : " + e); // 예외 발생 시 오류 메시지 출력
		}
	}
    ...
    public boolean updateIncome(IncomeBean ibean) {
        boolean result = false; // 결과를 저장할 변수 초기화
        try {
            conn = ds.getConnection(); // 데이터베이스 연결 설정
            String sql = "UPDATE income SET income_teamcode = ?, income_briefs = ?, income_ticket = ?, income_goods = ?, income_fan = ?, income_broad = ?, income_spon = ?, income_ad = ?, income_rent = ?, income_loan = ?, income_pysell = ? WHERE income_code = ?"; // SQL 업데이트 쿼리 준비
            pstmt = conn.prepareStatement(sql); // 준비된 명령문 생성
            pstmt.setString(1, ibean.getTeamcode()); // 팀 코드 설정
            pstmt.setString(2, ibean.getBriefs()); // 적요 설정
            pstmt.setInt(3, ibean.getTicket()); // 티켓 수입 설정
            pstmt.setInt(4, ibean.getGoods()); // 굿즈 수입 설정
            pstmt.setInt(5, ibean.getFan()); // 팬 수입 설정
            pstmt.setInt(6, ibean.getBroad()); // 방송권 수입 설정
            pstmt.setInt(7, ibean.getSpon()); // 스폰서 수입 설정
            pstmt.setInt(8, ibean.getAd()); // 광고 수입 설정
            pstmt.setInt(9, ibean.getRent()); // 대여료 수입 설정
            pstmt.setInt(10, ibean.getLoan()); // 임대 수입 설정
            pstmt.setInt(11, ibean.getPysell()); // 선수판매 수입 설정
            pstmt.setInt(12, ibean.getCode()); // 수입 코드 설정
            int rows = pstmt.executeUpdate(); // 쿼리 실행 및 영향받은 행 수 반환
            if (rows > 0) {
                result = true; // 업데이트 성공 시 결과를 true로 설정
            }
        } catch (Exception e) {
            System.out.println("updateIncome err : " + e); // 예외 발생 시 오류 메시지 출력
        } finally {
            try {
