...
public class InjuryMgr {
	...
	public ArrayList<InjuryBean> getAllInjuries(String teamcode) {
        ArrayList<InjuryBean> list = new ArrayList<InjuryBean>();
        try {
            conn = ds.getConnection();
            String sql = "SELECT * FROM injury inner join player where injury_bn=player_bn and player_teamcode=?";
	...
	public boolean isInjuryExist(int bn, String teamcode) {
	    boolean isExist = false;
	    try {
	        conn = ds.getConnection();
	        String sql = "SELECT COUNT(*) FROM injury inner join player where injury_bn=player_bn and injury_bn = ? and player_teamcode=?";
	        pstmt = conn.prepareStatement(sql);
	        pstmt.setInt(1, bn);
	...
	public InjuryBean getInjuryByBn(int bn, String teamcode) {
	    InjuryBean injury = null;
	    try {
	        conn = ds.getConnection();
	        String sql = "SELECT * FROM injury inner join player where injury_bn=player_bn and injury_bn = ? and player_teamcode=?";
	        pstmt = conn.prepareStatement(sql);
	        pstmt.setInt(1, bn);
	        pstmt.setString(2, teamcode);
	        rs = pstmt.executeQuery();
